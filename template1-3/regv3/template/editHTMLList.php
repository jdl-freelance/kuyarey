<?php 
    ...
    if($_POST['ListID']==null){
        ...
        $sql_s = "Select * from wm_webinars_lists where `Visibility`=1 AND `TemplateID`=3";
        ...
        mysql_select_db($db,$conn);
        $sql_s = "Select * from wm_webinars_lists";
        $query = mysql_query($sql_s);
        $count = mysql_num_rows($query);
        if($count != null){
            while($row = mysql_fetch_assoc($query)){
        ...
        mysql_select_db($db,$conn);
        $sql_s = "Select * from wm_webinars_lists where ListID ='$_POST[ListID]' AND Visibility=1 AND `TemplateID`=3";
        $query = mysql_query($sql_s);
        $count = mysql_num_rows($query);
