<?php
    ...
    if(isset($_GET['ContentName'])){
        ...
        $sql_s = "Select * from wm_webinars_content where ContentName='$ContentName' AND Visibility=1 AND `TemplateID`=7";
    ...
    if(isset($_POST['ContentName'])){
        ...
        $ContentText = mysql_real_escape_string($_POST['ContentText']);
        mysql_select_db($db,$conn);
        $sql_s = "Update wm_webinars_content SET `ContentText`='$ContentText' where `ContentName`='$ContentName' AND `TemplateID`=7";
       t;
        mysql_query($sql_s);
    ...
    if(isset($_POST['SaveList'])){
        ...
        for($i=0;$i<count($arrID);$i++){
            $ListID = $arrID[$i];
            $sql_s = "SELECT COUNT(`ListID`) as ListID FROM `wm_webinars_lists` WHERE `ListID`='$ListID' AND `TemplateID`=7";
            ...
            if($count==1){
                ...
                $Text = mysql_real_escape_string($arrData[$i][0]);
                $Icon = $arrData[$i][1];
                if($Icon!=null || $Text!=null){
                    $sql_i="Insert into wm_webinars_lists (`ListText`,`ListIcon`,`Visibility`,`TemplateID`) values ('$Text','$Icon',1,6)";
                    mysql_query($sql_i);
                }
    ...
    if(isset($_POST['DeleteContent'])){
        ...
        mysql_select_db($db,$conn);
        $sql_u = "Update `wm_webinars_content` SET `Visibility`='0' where `ContentName`='$ContentName' AND `TemplateID`=7";
        mysql_query($sql_u);
        echo "$sql_u";
    }
