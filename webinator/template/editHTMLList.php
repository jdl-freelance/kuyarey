<?php 
    ...
    if($_POST['ListID']==null){
        ...
        mysql_select_db($db,$conn);
        $sql_s = "Select * from wm_webinars_lists where `TemplateID`=2";
        $query = mysql_query($sql_s);
        $count = mysql_num_rows($query);
        if($count != null){
            while($row = mysql_fetch_assoc($query)){
        ...
        mysql_select_db($db,$conn);
        $sql_s = "Select * from wm_webinars_lists where ListID ='$_POST[ListID]' AND Visibility=1 AND `TemplateID`=2";
        $query = mysql_query($sql_s);
        $count = mysql_num_rows($query);
...
<div class="containerLI" id="<?php echo "list".$listID; ?>">
    <div class="containerIcon">
        ...
        <div>
            <input type="text" id="<?php echo "iconFileName".$listID; ?>" hidden="hidden"/>
...
$("#btnAddList").click(function(){
    ...
    var htmlListLayout = "<div class='containerLI' id='list"+id+"'><div class='containerIcon'><div><img id='icon"+id+"' src='' style='width:58px;height:58px;float:left;'/></div><div><input type='text' id='iconFileName"+id+"' hidden='hidden'/> <a href='javascript:;' class='linkUpload' id='a"+id+"' onclick='showLightBox(this.id)' style=''>Upload</a></div></div><div><textarea id='tb"+id+"' class='txtArea1' rows='3'></textarea></div><a href='javascript:;' class='aDelete' id='dl"+id+"' onclick='deleteList2(this.id)'>Delete</a></div>";
