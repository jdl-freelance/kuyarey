<?php
    ...
    if(isset($_GET['ContentName'])){
        ...
        $sql_s = "Select * from wm_webinars_content where ContentName='$ContentName' AND Visibility=1 AND `TemplateID`=6";
    ...
    if(isset($_POST['ContentName'])){
        ...
        mysql_select_db($db,$conn);
        $sql_s = "Update wm_webinars_content SET `ContentText`='$ContentText' where `ContentName`='$ContentName' AND `TemplateID`=6";
       t;
        mysql_query($sql_s);
    ...
    if(isset($_POST['SaveList'])){
        ...
        for($i=0;$i<count($arrID);$i++){
            $ListID = $arrID[$i];
            $sql_s = "SELECT COUNT(`ListID`) as ListID FROM `wm_webinars_lists` WHERE `ListID`='$ListID' AND `TemplateID`=6";
    ...
    if(isset($_POST['DeleteContent'])){
        ...
        mysql_select_db($db,$conn);
        $sql_u = "Update `wm_webinars_content` SET `Visibility`='0' where `ContentName`='$ContentName' AND `TemplateID`=6";
        mysql_query($sql_u);
        echo "$sql_u";
    }
